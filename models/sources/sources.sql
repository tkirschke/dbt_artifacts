...
select
    ...
    cast(null as {{ type_string() }}) as {% if target.type == "sqlserver" or target.type == "fabric" %} "database"
    {% else %} database
    {% endif %},
    cast(null as {{ type_string() }}) as {% if target.type == "sqlserver" or target.type == "fabric" %} "schema"
    {% else %} schema
    {% endif %},
