with
    ...
    latest_model_stats as (
        ...
        select
            ...
            max(
                case
                    when was_full_refresh {% if target.type == "sqlserver" or target.type == "fabric" %} = 1 {% endif %}
            ...
            max(
                case
                    when not was_full_refresh {% if target.type == "sqlserver" or target.type == "fabric" %} = 1 {% endif %}
                    then query_completed_at
                end
